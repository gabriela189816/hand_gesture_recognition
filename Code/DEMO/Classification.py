...
def classify(classes, test_image, reduction, reduced_data_set, average_hands_flatten):
    ...
    for cl in range(np.shape(classes)[0]):
        ...
        if euclidean_distance[0, cl] < comparison:
            ...
            pass
    print("Comparación de distancia euclideana con respecto a cada clase:", euclidean_distance)
    print(f"La distancia Euclideana más corta es: {comparison} y pertenece a la clase {classes_name[position]}")
