...
for i in range(m):
    IMG[:,:,i] = cv.imread('Frames_Palm_Gesture/frame_0' + str(i) + '.png', cv.IMREAD_GRAYSCALE)      # Read the image in gray scale
...
for i in range(m): 
    M[i,:] = IMG[:,:,i].flatten('F') #Using Flatten function on array 1 to convert the multi-dimensional
